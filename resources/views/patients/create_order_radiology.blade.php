                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="container">
                                <div class="row">
                                    <div class="col-8">
                                        <h1 class="mb-5">{{ __('messages.choose_medical_radiology') }}</h1>
                                    </div>
                                    <div class="col-4">
                                ...
                                <ul class="list-group">
                                    <form action="{{ route('patients.store.order.radiology') }}" method="post">
                                        @csrf
                                        @method('post')
                                        <input type="number" name="patient_id" value="{{ $id }}" hidden>
                                        <div class="row">

                                        @foreach ($categoryRadiologies as $item)
                                           <div class="col-3 mb-3">
                                                ...
                                                <div id="" class="" style="margin-left: 10px;">
                                                    @foreach ($item->radiologies as $item1)
                                                        <li><input class="analysisCheckbox" type="checkbox" name="radiologies[]" value="{{ $item1->id }}" data-text="{{ $item1->name }}">  {{ $item1->name }}</li>
                                                    @endforeach
                                                    {{-- <ul class="list-group">
                                                        @foreach ($item->radiologies as $item1)
                                                            <li class="list-group-item">
                                                                <label>
                                                                    <input type="checkbox" name="radiologies[]" value="{{ $item1->id }}">  {{ $item1->name }}
                                                                </label>
                                                            </li>
                                                        @endforeach
                                                    </ul> --}}
