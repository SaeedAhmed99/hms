@extends('layouts.app')
@section('title')
    {{__('messages.radiology_technician_category')}}
@endsection
@section('css')
        ...
        <div class="d-flex flex-column">
            <div wire:id="WpskoqwzxJ5BdNxsPOsu" id="datatable-WpskoqwzxJ5BdNxsPOsu">
                ...
                <div class="container-fluid">
                    <div class="d-flex flex-column">
                      ...
                      <div class="d-md-flex justify-content-between mb-3 livewire-search-box align-items-center">
                        <div class="d-md-flex">
                            <h1 class="text-gray-900 mb-0">{{ __('messages.radiology_technician_category_add_type') }}</h1>
                        </div>
                        ...
                        <div class="d-flex justify-content-end ">
                          <div class="dropdown">
                            ...
                            <a href="{{ route('radiology.category.index') }}" class="btn btn-primary">{{ __('messages.common.back') }}</a>
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                              <li>
                                <button data-bs-toggle="modal" data-bs-target="#radiologyCategoryModal" class="dropdown-item  px-5">{{ __('messages.radiology_technician_category_add_type') }}</button>
                              </li>
                            </ul>
                          </div>
                        </div>
                ...
                <div class="table-responsive">
                    ...
                    <tbody class="">
                        @foreach ($radiologies as $radiology)
                            <tr id="Category" wire:loading.class.delay="" class="" wire:key="row-0-WpskoqwzxJ5BdNxsPOsu">
                                <td class="" wire:key="cell-0-2-WpskoqwzxJ5BdNxsPOsu">{{ $radiology->name }}</td>

                                <td class="" wire:key="cell-0-2-WpskoqwzxJ5BdNxsPOsu">{{ $radiology->price }}</td>

                                <td class="" wire:key="cell-0-2-WpskoqwzxJ5BdNxsPOsu">
                                    <button data-bs-toggle="modal" data-id="{{ $radiology->id }}" data-bs-target="#updateCategoryModal" title="{{__('messages.common.edit') }}"
                                        class="btn px-1 text-primary fs-3 ps-0 edit-btn">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </button>   
                                    <button title="<?php echo __('messages.common.delete') ?>" data-id="{{ $radiology->id }}"
                                        class="radiologyCategory-delete-btn btn px-1 text-danger fs-3 pe-0" wire:key="">
                  ...
                  <!-- Modal -->
                  <div id="radiologyCategoryModal" class="modal fade" role="dialog" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <!-- Modal content-->
                        <div class="modal-content">
                            <div class="modal-header">
                                <h3 class="modal-title" id="exampleModalLabel">{{ __('messages.radiology_technician_category_add_type') }}</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                            </div>
                            <form id="addCategoryForm" action="{{ route('radiology.category.add.type.store') }}" method="post">
                              @csrf
                              @method('post')
               ...
               <!-- Modal -->
                <div id="updateCategoryModal" class="modal fade" role="dialog" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h3 class="modal-title" id="exampleModalLabel">{{ __('messages.bill.edit') }}</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                            </div>
                            <form id="updateCategoryForm" action="{{ route('radiology.category.add.type.update') }}" method="post">
                              @csrf
                              @method('post')
                              ...
                              <div class="modal-body">
                                    ...
                                    <div class="form-group col-sm-12 mb-5">
                                        ...
                                        <input name="price" id="price-input" class="form-control" id="">
                                    </div>
                                </div>
                                <div class="modal-footer pt-0">
...
@section('scripts')
    ...
    <script>
        listenClick('.radiologyCategory-delete-btn', function (event) {
            var id = $(this).data("id");
